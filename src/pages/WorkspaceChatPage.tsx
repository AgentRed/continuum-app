...
export default function WorkspaceChatPage({ palette, API_BASE }: WorkspaceChatPageProps) {
  ...
  useEffect(() => {
    // Load workspace name if needed
    if (workspaceId) {
      // TODO: Fetch workspace details to get name
  ...
  const handleSend = async () => {
    if (!inputValue.trim() || sending || !workspaceId) return;
    ...
    try {
      ...
      // Create conversation if needed (with workspaceId)
  ...
  return (
    <Stack gap="md">
      <PageHeaderCard
        title={workspaceName ? `Workspace Chat: ${workspaceName}` : "Workspace Chat"}
        subtitle="Chat with Continuum in this workspace"
        palette={palette}
        right={
          <ProviderSelector
            API_BASE={API_BASE}
        ...
        <Stack gap="md" style={{ flex: 1, overflow: "hidden" }}>
          ...
          <Group gap="md" align="flex-end">
            ...
            <Button
              ...
              onClick={handleSend}
              disabled={!inputValue.trim() || sending || !workspaceId}
              loading={sending}
              styles={{
                root: {
